...
classDiagram
    ...
    ProvisionStep <|.. AssignRecommendationModelsStep
    ProvisionStep <|.. SetupDefaultBrandingPreferencesStep
    ProvisionStep <|.. SetupDefaultPrmPreferencesStep
    ProvisionStep <|.. SetupRfsUiPreferencesStep
    ProvisionStep <|.. SetupDefaultVocabulariesInCeStep
    ProvisionStep <|.. SetupDefaultDatasourcesStep
    ProvisionStep <|.. SetupDefaultCitationStylesStep
...
sequenceDiagram
    ...
    participant WS as ProvisionWorkflowService
    participant SE as StepExecutor
    participant Step as AssignRecommendationModelsStep
    participant Ext as ExternalOrganizationClient
    participant FT as StepFailureTranslator
    participant DB as H2
    participant Log as Application log

    WS->>SE: execute(jobId, ASSIGN_RECOMMENDATION_MODELS, ctx)
    SE->>DB: UPDATE step ASSIGN_RECOMMENDATION_MODELS status=IN_PROGRESS
    SE->>Step: execute(ctx)
    Step->>Ext: assignRecommendationModels(orgId)
    ...
    SE->>DB: UPDATE step ASSIGN_RECOMMENDATION_MODELS status=FAILED, errorCode, errorMessage
    SE->>Log: log.error(step, job, code, message, stacktrace)
    SE--x WS: StepExecutionException(ASSIGN_RECOMMENDATION_MODELS)
    WS->>DB: UPDATE job status=FAILED, finishedAt
    Note over DB: SETUP_DEFAULT_BRANDING_PREFERENCES..SETUP_FSP_BOOSTERS remain NOT_STARTED (pre-seeded)
```
